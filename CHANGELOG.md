...
- **Startup markdown→SQLite reconcile is gated per scope** ([#227](https://github.com/chandra447/pi-hermes-memory/issues/227)): `session-start.persistence-sync` previously parsed every markdown file and re-upserted every entry into SQLite on every launch, measured at 2.1–7.2s on large stores. Each markdown scope now carries a fingerprint (`extension_metadata`, written in the same transaction as the reconcile), so an unchanged scope skips the per-entry SQL and an unchanged store's startup cost drops to a directory walk plus a few indexed reads per scope. The fingerprint binds markdown bytes and a row count, not the mirror's content, so mirror drift that preserves the count — a hand-edited row, or a search index that lost rows — now waits for a repair instead of being rewritten on every launch; `/memory-sync-markdown` is the forced full repair for both. New and deleted project dirs are still detected by the walk, and a corruption rebuild re-mirrors markdown exactly once.
- **Compact flush runs on one shared window** ([#225](https://github.com/chandra447/pi-hermes-memory/issues/225)): the pre-compaction flush previously handed its full 30-second timeout to the direct transport and then again to the subprocess fallback, so a failing direct call re-sent the same large prompt to the subprocess and could spend ~60s saving nothing, silently. The flush now owns a single wall-clock window — new `flushCompactTimeoutMs` setting (default `60000`, raised from the hardcoded `30000`); the direct transport runs first and the subprocess receives only the remaining time. Escape still cancels immediately, and the shutdown flush keeps its own shorter silent cap.

### Fixed

- **Long session-search snippets are windowed around the match instead of a blunt head-slice**: the legacy `session_search` tool's truncation now centers each `snippetChars`-bounded window on the first hit of the query's effective term set (the same quoted-phrase-aware, stop-word-filtered terms the FTS5 match itself uses, per [#184](https://github.com/chandra447/pi-hermes-memory/pull/184)) instead of the first N characters, so a 6 MB legacy row surfaces the matched region rather than its preamble. `snippetChars` remains the per-result ceiling and the 50 KiB output cap, truncation notes, and `truncatedCount` accounting from #100 are unchanged — this changes only where the window opens. Window edges are nudged off UTF-16 surrogate-pair splits, an all-stop-word or unmatched anchor falls back to the head window (documented, not promised), and the store now selects `m.content` once instead of materializing the full body a second time per row. The unbounded full-body snippet detonation applies to releases predating #100's tool-level caps; on this head the change is snippet quality plus defense in depth.
...
- **A failed compact flush is now visible** ([#225](https://github.com/chandra447/pi-hermes-memory/issues/225)): when the flush cannot save before compaction — budget spent, child process killed by its watchdog, or a child error — the TUI shows one warning naming `flushCompactTimeoutMs` instead of failing silently. The watchdog-killed child path was invisible before. Setting `flushCompactTimeoutMs` to `0` or lower now skips the flush silently (an explicit disable, not a failure), and non-finite values fall back to the default.
- **Rapid Markdown memory writes now reuse a recent recovery snapshot** ([#214](https://github.com/chandra447/pi-hermes-memory/issues/214)): successive overwrites within one hour publish atomically without creating another `.recovery-*` sidecar, while slower writes still take a fresh snapshot for crash recovery.
...
- **Corruption rebuilds no longer carry markdown-scope fingerprints** ([#227](https://github.com/chandra447/pi-hermes-memory/issues/227)): the recovery copies coerce invalid values instead of dropping rows, so a copied fingerprint could agree with coerced rows while content drifted; a rebuilt database now re-mirrors markdown exactly once instead of trusting copied fingerprints.

...
export default function (pi: ExtensionAPI, config = loadConfig(), bootstrap: RuntimeBootstrapOptions = {}) {
  ...
  let projectName = "";
  const skillStore = bootstrap.skillStore ?? new SkillStore({
  ...
  const standingStore = Object.hasOwn(bootstrap, "standingStore")
    ? bootstrap.standingStore ?? null
  ...
  pi.on("session_start", async (_event, ctx) => {
    sessionContext = ctx;
    const startupPrepared = bootstrap.startupStoresPrepared?.() ?? false;
    if (!startupPrepared) {
  ...
  const seedBackgroundState = setupBackgroundReview(pi, store, projectStoreRef, config, {
    dbManager,
    projectName: projectNameRef,
    ensureMemoryReady,
    initialState: bootstrap.lazyState,
  });

  // ── 6. Setup session-end flush ──
  const seedFlushState = setupSessionFlush(pi, store, projectStoreRef, config, dbManager, projectNameRef, {
    ensureMemoryReady,
    initialUserTurnCount: bootstrap.lazyState?.userTurnCount,
  });
  ...
  return {
    seedLazyState(state: LazyEventState): void {
      seedBackgroundState(state);
      seedFlushState(state);
    },
  };
}

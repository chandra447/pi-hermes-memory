...
/**
 * Minimum fraction of significant query terms a document must contain to keep
 * an OR-fallback hit.
 */
...
const ASCII_WORD_TERM = /^[a-z0-9_]+$/;

/**
 * Does `haystack` contain `needle` as a term rather than as a coincidental
 * substring?
 *
 * ASCII word terms are matched on word boundaries, because a bare substring
 * test is strictly looser than the FTS match it post-filters: unicode61
 * tokenizes "uses" as one token, so `MATCH "use"` never hits it, yet
 * "uses".includes("use") does. Short terms are where this bites — "do" hits
 * inside "docker", "use" inside "uses" — which is exactly the low-precision
 * behaviour this filter exists to prevent.
 *
 * Anything else (CJK runs, punctuation) has no boundary to anchor on and
 * falls back to a substring test.
 */
function textContainsTerm(haystack: string, needle: string): boolean {
  if (!ASCII_WORD_TERM.test(needle)) return haystack.includes(needle);
  ...
  while (true) {
    ...
    if (!ASCII_WORD_TERM.test(before) && !ASCII_WORD_TERM.test(after)) return true;
    from = at + 1;
  }
}
...
export function countTermHitsInText(text: string, terms: string[]): number {
  ...
  for (const term of terms) {
    ...
    if (!needle) continue;
    if (textContainsTerm(haystack, needle)) hits += 1;
  }
  return hits;
}
...
/**
 * Term selection for the natural-language recovery paths (raw operator query
 * failed to parse, or every word was a stopword). Prefer significant terms so
 * recovery ranks exactly like the plain NL spelling; when the stopword list
 * eats the whole query, degrade to all word tokens rather than losing recall
 * entirely — a memory can legitimately be *about* "memory search". Pure
 * punctuation/symbol tokens are dropped (unicode61 cannot MATCH them).
 */
export function collectNaturalLanguageSearchTerms(query: string): string[] {
  const trimmed = query.trim();
  if (trimmed.length === 0) return [];
  ...
  return collectNaturalLanguageTerms(trimmed).filter(
    (term) => !/^[\p{P}\p{S}]+$/u.test(term),
  );
}
...
export function normalizeNaturalLanguageFts5Query(query: string): string {
  return collectNaturalLanguageSearchTerms(query).map(quoteFtsTerm).join(" ");
}

/**
 * Build the broader OR fallback for the same recovery paths. Callers must
 * post-filter hits by term coverage (passesTermHitFilter) so all-stopword
 * queries cannot OR-explode into casual single-word mentions.
 */
export function buildNaturalLanguageFallbackQuery(query: string): string | null {
  const terms = collectNaturalLanguageSearchTerms(query);

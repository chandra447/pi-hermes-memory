...
Direct calls try the configured `llmFallbackModels` chain before falling back to the legacy `pi -p --no-session` subprocess path for unresolved model, auth, provider, or parsing failures. Consolidation also falls back when the proposed operations do not free space, but an empty completion after the direct model chain is exhausted reports failure without spawning a child.

The automatic over-capacity consolidator triggered from `MemoryStore` itself always uses the subprocess path, since it runs without extension-runtime access. Consolidation subprocesses try configured fallback models in order for overload and recognized retryable provider/transport failures, not arbitrary failures. Before another paid attempt, the parent reloads storage: shrink relative to the snapshot actually sent to the child counts as progress and prevents replay. Cancellation stops further model attempts; exhausted failures are reported rather than treated as success.
...
| `llmFallbackModels` | unset | Ordered array of `provider/model` alternatives for direct LLM calls and consolidation subprocess recovery; see [Direct-Transport LLM Calls](#direct-transport-llm-calls-review-flush-correction-consolidation) for retry and terminal-response behavior |
...
| `consolidationTimeoutMs` | `180000` | Timeout in milliseconds per consolidation model attempt (auto and `/memory-consolidate` alike), not a total wall-clock ceiling across fallback attempts or targets. Configured values are used verbatim; a consolidation pays child-process boot plus a full LLM turn, so values below the default are frequently killed mid-run and log a warning at startup |
